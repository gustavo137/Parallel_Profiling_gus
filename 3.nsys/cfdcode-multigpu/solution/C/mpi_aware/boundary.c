#include <stdio.h>
#include <mpi.h>
#include <accel.h>
#include "boundary.h"

//grid is parallelised in the x direction

void boundarypsi(double **psi, int m, int n,
		 int b, int h, int w, MPI_Comm comm)
{

  int size, rank,i,j;
  int istart, istop;

  MPI_Comm_size(comm,&size);
  MPI_Comm_rank(comm,&rank);

  istart = m*rank + 1;
  istop = istart + m -1;

  //BCs on bottom edge

  for (i=b+1;i<=b+w-1;i++)
    {
      if (i >= istart && i <= istop)
	{
	  psi[i-istart+1][0] = (double)(i-b);
        }
    }

  for (i=b+w;i<=m*size;i++)
    {
      if (i >= istart && i <= istop)
	{
	  psi[i-istart+1][0] = (double)(w);
	}
    }

  //BCS on RHS

  if (rank == size-1)
    {
      for (j=1; j <= h; j++)
	{
	  psi[m+1][j] = (double) w;
	}

      for (j=h+1;j<=h+w-1; j++)
	{
	  psi[m+1][j]=(double)(w-j+h);
	}
    }
}

void haloswap(double **x, int m, int n, MPI_Comm comm)
{
  int rank, size;
  int tag=1;

  MPI_Status status;

  MPI_Comm_rank(comm,&rank);
  MPI_Comm_size(comm,&size);
  //no need to halo swap if serial:

  if (size > 1)
    {
      //send right boundaries and receive left ones

      if (rank == 0)
	{
	double * ptx = &x[m][1];

#pragma acc host_data use_device(ptx)
{
	  MPI_Send(ptx,n,MPI_DOUBLE,rank+1,tag,comm);
}  
	}
      else if (rank == size-1)
	{
	 double * ptx = &x[0][1];
#pragma acc host_data use_device(ptx)
{

	  MPI_Recv(ptx,n,MPI_DOUBLE,rank-1,tag,comm,&status);
}

	}
      else
	{
	double * ptx1 = &x[m][1];
	double * ptx2 = &x[0][1];

#pragma acc host_data use_device(ptx1,ptx2)
{

	  MPI_Sendrecv(ptx1,n,MPI_DOUBLE,rank+1,tag, ptx2,n,MPI_DOUBLE,rank-1,tag,     comm,&status);
}
	}

      //send left boundary and receive right

      if (rank == 0)
	{
	double * ptx = &x[m+1][1];
#pragma acc host_data use_device(ptx)
{
	  MPI_Recv(ptx,n,MPI_DOUBLE,rank+1,tag,comm,&status);
}
	}
      else if (rank == size-1)
	{
         double * ptx = &x[1][1];
#pragma acc host_data use_device(ptx)
{
	  MPI_Send(ptx,  n,MPI_DOUBLE,rank-1,tag,comm);
}
	}
      else
	{
        double * ptx1 = &x[1][1];
        double * ptx2 = &x[m+1][1];

#pragma acc host_data use_device(ptx1,ptx2)
{
	  MPI_Sendrecv(ptx1,  n,MPI_DOUBLE,rank-1,tag,ptx2,n,MPI_DOUBLE,rank+1,tag,  comm,&status);
}
	}
    }
}
